...
const Comments = (props) => {
  ...
  useEffect(() => {
    ...
    onValue(recentPostsRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
        data = snapshot.val();
        const hasil = Object.keys(data).map(
          (key) => [Number(key), data[key]][1]
        );
  ...
  return (
    <div id={mode.theme} className="comment">
      <form
        noValidate={false}
        autoComplete="off"
        onSubmit={(e) => {
              ...
              <Box key={idx} sx={{ pt: 2, pb: 3, px: 1 }}>
                <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                  ...
                  >
                    ...
                    >
                      <Moment fromNow unix>
                        {userComment.created_at / 1000}
                      </Moment>
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{ fontSize: "1rem", color: "black", mt: 1 }}
                    >
                      {userComment.body}
                    </Typography>
                  </Box>
